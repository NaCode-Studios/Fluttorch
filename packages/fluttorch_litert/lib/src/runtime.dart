...
/// Runs models exported for LiteRT.
///
/// The seam below this is the same C ABI ExecuTorch and ONNX Runtime answer to,
/// so this class is thin on purpose: what a third runtime changes is the engine,
/// not the contract. The manifest still says what the tensors are, the goldens are still
...
final class LiteRtRuntime implements FluttorchRuntime {
  const LiteRtRuntime(this._bindings);
  ...
  Future<LoadedModel> load({
    ...
    // flatbuffer reader. Refused here, where the reason is still legible.
    if (manifest.runtime != 'litert') {
    ...
    return _LiteRtModel(native, manifest);
  }
}

final class _LiteRtModel implements LoadedModel {
  _LiteRtModel(this._native, this.manifest);
  ...
  Future<TappedRun> runWithTaps(
    ...
    // Reported absent rather than approximated. A LiteRT graph carries no
    // handle for an intermediate, so reading one means marking it an output
    // before conversion: that produces a different artifact from the one the
    // goldens describe, and a gate handed the wrong tensor attributes a drift
    // to the wrong layer.
    throw CapabilityUnavailableException(
      backend: backend,
      capability:
          'activation taps: a LiteRT graph carries no handle for an '
          'intermediate, so reading one changes the artifact',
    );
  }
