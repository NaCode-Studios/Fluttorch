...
final _multiIo = Directory('../../testdata/multi_io_litert');

void main() {
  if (!_library.existsSync() || !_export.existsSync()) {
    test('the LiteRT gate needs tool/build_native.sh', () {}, skip: true);
    return;
  }
  ...
  group('two inputs and two outputs, kept apart', () {
    ...
    setUpAll(() async {
      ...
      model = await runtime.load(
        artifact: await File('${_multiIo.path}/multi_io.tflite').readAsBytes(),
